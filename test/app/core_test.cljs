...
(defn get-app-store-content [db]
  (get-in db [::store/content ::store/apps]))
...
(deftest app-meta-test
  (let [app-meta (app/new-app-meta "app-1" "simple app")]
    ...
    (testing "save new app-meta in repository"
      (let [app-db (store/add-new-meta db/default-db [app-meta])
    ...
    (testing "save no app into repository"
      (let [db' (store/add-new-meta db/default-db [])]
        (is (= db' db/default-db))))

    (testing "save few app-meta into repository"
      (let [app-db (store/add-new-meta db/default-db
                                     [(app/new-app-meta "app-1" "simple app 1")
                                      (app/new-app-meta "app-2" "simple app 2")])
            store-content (get-app-store-content app-db)]
        (is (= 2 (count store-content)))
        (is (= {::app/id "app-1" ::app/name "simple app 1" ::app/launcher {}}
              (store/find-app-by-id app-db "app-1")))
        (is (= {::app/id "app-2" ::app/name "simple app 2" ::app/launcher {}} 
              (store/find-app-by-id app-db "app-2")))))

    (testing "find application meta by id"
      (let [app-db (store/add-new-meta db/default-db [app-meta])
            found (store/find-app-by-id app-db "app-1")]
        (is (= found app-meta))))))
